...
import datetime
import arxiv
import openai, tenacity  #这个模块设置网络不通的时候自动重复的次数.
...
class Reader:
    ...
    def __init__(self, key_word, query, filter_keys, 
        ...
        # 读取配置文件
        self.config.read('ChatPaper-main/apikey.ini')
        # 获取某个键对应的值        
    ...
    def chat_method(self, text, method_prompt_token = 800):
        ...
        self.cur_api += 1
        self.cur_api = 0 if self.cur_api >= len(self.chat_api_list)-1 else self.cur_api
        text_token = len(self.encoding.encode(text)) #编码后的长度.
        clip_text_index = int(len(text)*(self.max_token_num-method_prompt_token)/text_token)
        clip_text = text[:clip_text_index]        #进行了阶段.......只有前1000个左右字符进去了.
        messages=[
...
if __name__ == '__main__':    
    ...
    parser.add_argument("--pdf_path", type=str, default=p, help="if none, the bot will download from arxiv with query")
